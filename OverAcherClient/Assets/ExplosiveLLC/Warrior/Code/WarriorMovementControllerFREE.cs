    ...
    public class WarriorMovementControllerFREE : SuperStateMachine
    {
        ...
        protected override void LateGlobalSuperUpdate()
        {
            ...
            if (canMove)
            {
                ...
                {
                    isMoving = true;
                    CmdMove(isMoving, currentVelocity.magnitude);
                }
                else
                {
                    isMoving = false;
        ...
        [Command]
        private void CmdMove(bool isMoving, float magnitude)
        {
            RpcMove(isMoving, magnitude);
        }

        [ClientRpc]
        private void RpcMove(bool isMoving, float magnitude)
        {
        ...
        private void Jump_SuperUpdate()
        {
            ...
            planarMoveDirection = Vector3.MoveTowards(planarMoveDirection,
                warriorInputController.moveInput * inAirSpeed, jumpAcceleration * superCharacterController.deltaTime);
        ...
        [Command]
        private void CmdFallExit()
        {
            RpcFallExit();
        }

        [ClientRpc]
        private void RpcFallExit()
        {
            animator.SetInteger("Jumping", 0);
            animator.SetTrigger("JumpTrigger");
        }
        ...
        public void Dash(int dash)
        {
            ...
            warriorController.Lock(true, true, true, 0,
                warriorController.warriorTiming.TimingLock(warriorController.warrior, "dash"));
        }
        ...
        private void RotateTowardsMovementDir()
        {
            if (warriorInputController.moveInput != Vector3.zero)
            {
                transform.rotation = Quaternion.Slerp(transform.rotation,
                    Quaternion.LookRotation(warriorInputController.moveInput), Time.deltaTime * rotationSpeed);
            }
        }
